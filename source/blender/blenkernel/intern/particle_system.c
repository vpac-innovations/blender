...
static int tot_particles(ParticleSystem *psys, PTCacheID *pid)
{
	if (pid && psys->pointcache->flag & PTCACHE_EXTERNAL)
		...
		return psys->part->grid_res * psys->part->grid_res * psys->part->grid_res - psys->totunexist + psys->totadded;
	else
...
void integrate_particle(ParticleSettings *part, ParticleData *pa, float dtime, float *external_acceleration,
			...
			if (part->fluid->solver == SPH_SOLVER_DDR) {
			  ...
			  if(part->fluid->flag & SPH_FAC_ADPTV_RES){
				  ...
				  * Bonet 2007 and Vacondio et al. 2013. */
				  ...
				  if ((cfra > 9 && cfra < 50000) && pa->adptv == PARS_ADAPTABLE){
